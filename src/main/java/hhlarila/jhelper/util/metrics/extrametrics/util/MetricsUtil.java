...
package hhlarila.jhelper.util.metrics.extrametrics.util;
 ...
 *
 */
public class MetricsUtil<S extends Solution<?>> {
     ...
     */
    public List<S> readSolutionSet(String path) {
        try {
            Front front = new ArrayFront(path);
            return (List<S>) FrontUtils.convertFrontToSolutionList(front);
        } catch (FileNotFoundException ex) {
            Logger.getLogger(MetricsUtil.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
    } // readSolutionSet
     ...
     * @return A solution set
     */
    public List<S> readNonDominatedSolutionSet(String path) {
        List<S> solutionset = readSolutionSet(path);
    ...
    public void readNonDominatedSolutionSet(String path,
            List<S> solutionSet) {
        List<S> toRead = readNonDominatedSolutionSet(path);
        solutionSet.addAll(toRead);
        List<S> aux = SolutionListUtils.getNondominatedSolutions(solutionSet);
        solutionSet.clear();//no returning method, so keep the pointer
        solutionSet.addAll(aux);
    }
     ...
     *
     */
    public double[] hvContributions(List<S>[] populations) {
        boolean empty = true;
        for (List<S> population2 : populations) {
        ...
        for (List<S> population1 : populations) {
            size += population1.size();
        }

        // allocating space for the union
        union = new ArrayList<>(size);

        // filling union
        for (List<S> population : populations) {
        ...
        frontValues[0] = FrontUtils.convertFrontToArray(new ArrayFront(union));
        for (int i = 0; i < populations.length; i++) {
            if (populations[i].size() > 0) {
                frontValues[i + 1] = FrontUtils.convertFrontToArray(new ArrayFront(populations[i]));
            } else {
        ...
        double[] maximumValues = FrontUtils.getMinimumValues(new ArrayFront(union));
        double[] minimumValues = FrontUtils.getMaximumValues(new ArrayFront(union));
     ...
     *
     */
    public double[] hvContributions(List<S> archive,
            List<S>[] populations) {
