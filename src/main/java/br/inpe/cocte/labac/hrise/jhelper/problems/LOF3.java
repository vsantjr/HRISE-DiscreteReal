...
public class LOF3 extends AbstractIntegerProblem {
     ...
     * @throws IOException 
     */
    public LOF3(int m) throws IOException {
        ...
        // Init Level
    	//String[] initStates = {"saveFileSelNameSaveBtn", "saveFileSelNameCancelBtn"};
    	...
    	// E level
    	//String[] levelE = {"setParagraphStyle", "setFontName", "setFontSize"};
    	
    	String[] levelE = {"setParagraphStyle", "setFontName", "setFontSize", "formatTextStyle", "toggleBulletedList", "toggleNumberedList", "alighParagraph"}; 
    	...
    	// Create the Edges: Init -> C; B -> D; D -> B; B -> F; D -> F;  B -> C; C -> B; C -> D; D -> C;  C -> E; E -> C; B -> E; E -> B;  D -> E; E -> D
    	...
    	directedGraph.addEdge(levelB, levelD);
    	directedGraph.addEdge(levelD, levelB);
    	directedGraph.addEdge(levelB, levelF);
    	directedGraph.addEdge(levelD, levelF);
    	...
    	for (int i = 0; i < levelC.length; i++) {
    		...
    		for (int j = 0; j < levelE.length; j++) {
    	    	...
    	    	directedGraph.addEdge(levelE[j], levelC[i]);
    	    }
    		//directedGraph.addEdge(levelC[i], levelF);
    	}
    	
    	for (int i = 0; i < levelE.length; i++) {
    		directedGraph.addEdge(levelB, levelE[i]);
    		directedGraph.addEdge(levelE[i], levelB);
    		directedGraph.addEdge(levelD, levelE[i]);
