...
public class LOF2 extends AbstractIntegerProblem {
     ...
     * @throws IOException 
     */
    public LOF2(int m) throws IOException {
     ...
     // Init Level
    	String initStates[] = {"typeText"};
    	
    	// B Level
    	//String[] levelB = {"setParagraphStyle", "setFontName", "setFontSize"};
    	
    	String[] levelB = {"setParagraphStyle", "setFontName", "setFontSize", "formatTextStyle", "toggleBulletedList", "toggleNumberedList", "alighParagraph"};
    	...
    	for (int i = 0; i < levelB.length; i++) {
    	  directedGraph.addVertex(levelB[i]);
    	}
    	...
    	// Create the Edges: Init -> B; B -> D; D -> B; B -> C; B -> B; B -> E; C -> E; E -> C; E -> D; E -> F
    
    	for (int i = 0; i < initStates.length; i++) {
    	  for (int j = 0; j < levelB.length; j++) {	
    		 directedGraph.addEdge(initStates[i],levelB[j]);
    	  }	
    	}
    	
    	for (int i = 0; i < levelB.length; i++) {
    		directedGraph.addEdge(levelB[i], levelD);
  		    directedGraph.addEdge(levelD, levelB[i]);
    		for (int j = 0; j < levelC.length; j++) {
    		  directedGraph.addEdge(levelB[i], levelC[j]);
    		  //directedGraph.addEdge(levelC[j], levelB[i]);
    		}
    	}
    	
    	
    	//for (int i = 0; i < levelB.length; i++) {
    		///for (int j = 0; j < levelD.length; j++) {
    		 // directedGraph.addEdge(levelB[i], levelD);
    		 // directedGraph.addEdge(levelD, levelB[i]);
    		//}
    	//}
    	...
    	for (int i = 0; i < levelB.length; i++) {
    		directedGraph.addEdge(levelB[i], levelB[i]);
    		for (int j = 0; j < levelE.length; j++) {
    		  directedGraph.addEdge(levelB[i], levelE[j]);
    		  //directedGraph.addEdge(levelE[j], levelB[i]);
    		}
    	}
    	...
    	//for (int i = 0; i < levelB.length; i++) {
			...
			//directedGraph.addEdge(levelB[i],levelF);
		//}
    	
    	for (int i = 0; i < levelE.length; i++) {
    		for (int j = 0; j < levelC.length; j++) {
    			directedGraph.addEdge(levelC[j],levelE[i]);
    			directedGraph.addEdge(levelE[i],levelC[j]);
    		}
    		directedGraph.addEdge(levelE[i],levelD);
