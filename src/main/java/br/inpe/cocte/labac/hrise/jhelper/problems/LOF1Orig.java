...
public class LOF1Orig extends AbstractIntegerProblem {
   ...
   // private Set<String> terminalVertices = new LinkedHashSet<String>();
     ...
     * @throws IOException 
     */
    public LOF1Orig(int m) throws IOException {
     ...
     // Init Level
    	...
    	for (int i = 0; i < initStates.length; i++) {
    		...
    		directedGraph.addEdge(initStates[i],levelB);
    		directedGraph.addEdge(levelB, initStates[i]);
    	}
    	...
    	//for (int i = 0; i < levelE.length; i++) {
    		//directedGraph.addEdge(levelC, levelD[i]);
    		//for (int j = 0; j < levelC.length; j++) {
    			//directedGraph.addEdge(levelE[i], levelC[j]);
    			//directedGraph.addEdge(levelE[j],levelF);
    		//}
    	//}
